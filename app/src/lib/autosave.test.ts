...
describe('suppression and immediate save (startup restore)', () => {
  it('suppressed: notify and flush never write, even with new milestones', async () => {
    const { a, r } = rig()
    a.suppress(true)
    r.setMilestones(['c1'])
    a.notify()
  ...
  it('suppressed: saveNow is also refused', async () => {
    const { a, r } = rig()
    a.suppress(true)
    a.saveNow()
    await settle()
  ...
  it('unsuppressing restores normal triggers, including milestones missed meanwhile', async () => {
    const { a, r } = rig()
    a.suppress(true)
    r.setMilestones(['c1'])
    a.notify()
  ...
  it('saveNow writes immediately without a new milestone', async () => {
    const { a, r } = rig(['c1'])
    a.saveNow()
    await settle()
    expect(r.calls).toHaveLength(1)
